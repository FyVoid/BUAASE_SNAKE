...
def genBoard():
    ...
    for i, choice in enumerate(directions):
        ...
        if 0 <= next_head[0] < 8 and 0 <= next_head[1] < 8 \
                and (torch.all(board[next_head[0]][next_head[1]] == torch.tensor([1, 0, 0, 0, 0, 0, 0])) or torch.all(board[next_head[0]][next_head[1]] == torch.tensor([0, 0, 0, 0, 0, 0, 1]))):
